...
predsaver_obj_de = jnp.load("results/mlp_bike_mile_predsaver_de.npz", allow_pickle=True)
exp_id = "mlp_bike_mile"
# %%
target = predsaver_obj["target"]
target.shape
# %%
pred_dist_de = predsaver_obj_de["pred_dist"]
pred_dist_de.shape # (num_chains, 1, num_data, 2 (mean, logstd))
# %%
pred_dist = predsaver_obj["pred_dist"]
pred_dist.shape # (num_chains, num_samples, num_data, 2 (mean, logstd))
# %%
...
# %%
def get_preds(pred_dist):
    # sample a prediction from each of the distributions (gaussian and use exp for variance)
    means = pred_dist[..., 0]  # (num_chains, num_samples, num_data)
    logstd = pred_dist[..., 1]  # (num_chains, num_samples, num_data)
    stds = jnp.exp(logstd)
    # sample from the gaussian
    key = jax.random.PRNGKey(0)
    samples = means + stds * jax.random.normal(key, shape=means.shape)
    return samples  # (num_chains, num_samples, num_data)
# %%
full_ensemble_rmse = bmetrics.rmse(pred=get_preds(pred_dist_test), target=target_test)
full_chainwise_rmse = [
    bmetrics.rmse(
        pred=get_preds(pred_dist_test[chain_idx:chain_idx+1]),
        target=target_test
    )
...
chainwise_lppd = [
    bmetrics.lppd(
        bmetrics.lppd_pointwise(pred_dist=pred_dist_test[chain_idx:chain_idx+1], y=target_test, 
                                task=Task.REGRESSION).squeeze(0)
    )
    for chain_idx in range(pred_dist_test.shape[0])
]
...
chainwise_rmse_de = [
    bmetrics.rmse(
        pred=get_preds(pred_dist_de_test[chain_idx:chain_idx+1]),
        target=target_test
    )
    for chain_idx in range(pred_dist_de_test.shape[0])
]
print("Chainwise RMSE DE:", [float(rmse) for rmse in chainwise_rmse_de])
...
def get_logprob_up_to_k(pred_dist, target, k):
    ...
    logprob_pw = bmetrics.lppd_pointwise(pred_dist=pred_dist_k, y=target, task=Task.REGRESSION)
...
# %% now based on the early stopping sample sizes select samples only that have an evalue exceeding the threshold and compute test RMSE and lppd
...
for alpha in alphas:
    ...
    for chain_idx in range(evalues_chainwise.shape[0]):
        ...
        if early_stopping_sample == max_k:
            ...
            pred_dist_es = pred_dist_test[chain_idx:chain_idx+1, :early_stopping_sample, :, :]  # (1, k_es, num_data, 2 (mean, logstd))
            # compute test rmse
            test_rmse = bmetrics.rmse(
                pred=get_preds(pred_dist_es),
                target=target_test,
            )
            # compute test lppd
            test_lppd = bmetrics.lppd(
                bmetrics.lppd_pointwise(pred_dist=pred_dist_es, y=target_test, task=Task.REGRESSION)
            )
...
# aggregate the chainwise results both RMSE and lppd for the early stopping (per alpha) and full samples
for chain_idx in range(evalues_chainwise.shape[0]):
    pred_dist_full = pred_dist_test[chain_idx:chain_idx+1, :, :, :]  # (1, num_samples, num_data, 2 (mean, logstd))
    test_rmse_full = bmetrics.rmse(
        pred=get_preds(pred_dist_full),
        target=target_test,
    )
    test_lppd_full = bmetrics.lppd(
        bmetrics.lppd_pointwise(pred_dist=pred_dist_full, y=target_test, task=Task.REGRESSION)
    )
    results_early_stopping.append({
        "alpha": "full",
        "chain": chain_idx,
        "early_stopping_sample": max_k,
        "test_rmse": test_rmse_full,
...
# group by alpha and compute mean and std of test RMSE and lppd
...
summary_early_stopping_long_df = pd.melt(
    ...
    value_vars=["mean_test_rmse", "mean_test_lppd"],
    var_name="metric",
    value_name="mean_value"
)

# Add corresponding std columns
std_mapping = {"mean_test_rmse": "std_test_rmse", "mean_test_lppd": "std_test_lppd"}
    ...
    for chain_idx in range(evalues_chainwise.shape[0]):
        ...
        if early_stopping_sample != max_k:
            ...
            if no_early_stop_strategy == "none":
                ...
                pred_dist_es = pred_dist_test[chain_idx:chain_idx+1, :1, :, :]  # (1, 1, num_data, 2 (mean, logstd))
                pred_dists_es.append(pred_dist_es)
            else:
                raise ValueError("Invalid no_early_stop_strategy")
    pred_dist_es_ensemble = jnp.concatenate(pred_dists_es, axis=1)  # (num_chains, k_es_chain, num_data, 2 (mean, logstd))
    ensemble_rmse_es = bmetrics.rmse(
        pred=get_preds(pred_dist_es_ensemble),
        target=target_test,
    )
    ensemble_lppd_es = bmetrics.lppd(
        bmetrics.lppd_pointwise(pred_dist=pred_dist_es_ensemble, y=target_test, task=Task.REGRESSION)
    )
    ensemble_results_early_stopping.append({
        "alpha": alpha,
        "ensemble_rmse": ensemble_rmse_es,
        "ensemble_lppd": ensemble_lppd_es,
        "ensemble_members": pred_dist_es_ensemble.shape[1]
    })
ensemble_results_early_stopping.append({
    "alpha": "full",
    "ensemble_rmse": full_ensemble_rmse,
    "ensemble_lppd": full_ensemble_lppd,
    "ensemble_members": pred_dist_test.shape[0] * pred_dist_test.shape[1]
})
ensemble_results_early_stopping.append({
    "alpha": "DE",
    "ensemble_rmse": bmetrics.rmse(
        pred=get_preds(pred_dist_de_test),
        target=target_test,
    ),
    "ensemble_lppd": bmetrics.lppd(
        bmetrics.lppd_pointwise(pred_dist=pred_dist_de_test, y=target_test, task=Task.REGRESSION)
    ),
...
for col in ["ensemble_rmse", "ensemble_lppd", "ensemble_members"]:
...
ensemble_results_long_df = pd.melt(
    ...
    value_vars=["ensemble_rmse", "ensemble_lppd"],
    var_name="metric",
    value_name="value"
)
# Create cleaner metric labels
